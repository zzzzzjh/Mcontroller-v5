...
/************Noted*************
 *
 * 如果飞机不稳定,与传感器噪声、机架震动、机体质量分布有很大关系,
...
static bool get_mav_yaw=false, get_odom_xy=false, getodom_z=false;
static bool use_mag=true, mag_corrected=false, mag_correcting=false;
...
static float accel_filt_hz=20;//10-20HZ，高于20会震，低于10会迟钝
static float gyro_filt_hz=20;//10-20HZ
static float mag_filt_hz=2.5;//HZ
static float accel_ef_filt_hz=20;//HZ
...
EKF_Baro *ekf_baro=new EKF_Baro(_dt);
EKF_Rangefinder *ekf_rangefinder=new EKF_Rangefinder(_dt);
EKF_Odometry *ekf_odometry=new EKF_Odometry(_dt);
EKF_GNSS *ekf_gnss=new EKF_GNSS(_dt);
...
void update_dataflash(void){
	if(dataflash->get_addr_num_max()>1000){
		dataflash->reset_addr_num_max();
	}
	if(dataflash->get_addr_num_max()<=0){
...
static Vector3f lidar_offset=Vector3f(0.0f,0.0f, -16.0f);
static Vector3f odom_offset;
				...
				switch(cmd.command){
					...
					case MAV_CMD_DO_SET_PARAMETER:
						if(is_equal(cmd.param1,0.5f)){//reset PID parameters in flash
							...
							param->acro_y_expo.value=0;
							param->acro_yaw_p.value=4.5f;
							param->throttle_midzone.value=THR_MIDZ_DEFAULT;
							param->pilot_speed_dn.value=PILOT_VELZ_MAX;
							param->pilot_speed_up.value=PILOT_VELZ_MAX;
							...
							param->throttle_filt.value=0.05f;
...
#endif
	if((time_last_attitude>0)&&((HAL_GetTick()-time_last_attitude)>1000)){
		//off board 连上又断了
		get_mav_yaw=false;
	}
}
...
void ekf_xy_reset(void){
	ekf_odometry->reset();
	ekf_gnss->reset();
	pos_control->set_xy_target(get_pos_x(), get_pos_x());
}
...
//	usb_printf("gx:%f,gy:%f,gz:%f\n",gyro.x,gyro.y,gyro.z);
	...
	if(!initial_accel_gyro){
		...
		Vector3f accel_delt=accel_correct - accel_filt;
		Vector3f gyro_delt=gyro_correct - gyro_filt;
		if(accel_delt.length()<30){//震动最多会引起30m/s/s的误差，这一步是扔掉坏数，所以认为30以内都是好数
//			accel_filt = accel_filt + accel_delt * _filt_alpha(1.0f/400,accel_filt_hz);
			accel_filt=_accel_filter.apply(accel_correct);
		}else{
			accel_filt = Vector3f(0,0,-GRAVITY_MSS);
			ahrs->quaternion2.earth_to_body(accel_filt);
		}
//		usb_printf("accelx:%f,accely:%f,accelz:%f\n",accel_filt.x,accel_filt.y,accel_filt.z);
		if(gyro_delt.length()<10&&gyro.length()<10){
//			gyro_filt = gyro_filt + gyro_delt * _filt_alpha(1.0f/400,gyro_filt_hz);
...
void ahrs_update(void){
	...
	if(ahrs_stage_compass){
		...
		//由ahrs的四元数推出欧拉姿态角
		roll_rad = ahrs->quaternion2.get_euler_roll();
		roll_deg=roll_rad*RAD_TO_DEG;
		pitch_rad =ahrs->quaternion2.get_euler_pitch();
		pitch_deg=pitch_rad*RAD_TO_DEG;
		yaw_rad = ahrs->quaternion2.get_euler_yaw();
...
static float baro_alt_filt=0,baro_alt_init=0, baro_filt_hz=10;//HZ
...
void update_baro_alt(void){
	...
	if(!initial_baro){
		...
		baro_alt_filt = _baro_alt_filter.apply(baro_alt-baro_alt_init);
...
void ekf_baro_alt(void){
	...
	ekf_baro->update(get_baro_alt_filt);
}
...
void ekf_rf_alt(void){
	...
	ekf_rangefinder->update(get_rangefinder_data);
}
...
void ekf_odom_xy(void){
	...
	ekf_odometry->update(get_odom_xy);
}
...
void ekf_gnss_xy(void){
	...
	ekf_gnss->update(get_gnss_location);
}
...
static uint32_t preland_detector_count = 0;
...
void update_land_detector(void)
{
	...
	//******************落地前********************
	if(get_vel_z()<0){//如果在下降,那么接下来的100周期内开始检测降落
		preland_detector_count=0;
	}
	preland_detector_count++;
	if(preland_detector_count>2000){
...
//        bool rangefinder_check = (!rangefinder_alt_ok() || rangefinder_state.alt_cm_filt.get() < LAND_RANGEFINDER_MIN_ALT_CM);
        ...
        if (motor_at_lower_limit && descent_rate_low) {
            ...
            if( land_detector_count < ((float)LAND_DETECTOR_TRIGGER_SEC)/_dt) {
                land_detector_count++;
            } else {
                set_land_complete(true);
            }
...
void Logger_Cat_Callback(void){
	...
	osDelay(1);
	sdlog->Logger_Write("%8s %8s %8s %8s %8s %8s ",//LOG_POS_Z
			"barofilt", "alt_t", "pos_z", "vel_z", "rf_alt", "rf_alt_t");
	osDelay(1);
	...
	osDelay(1);
	sdlog->Logger_Write("%8s %8s %8s %8s %8s %8s ",//LOG_TARGET
			"pos_x_t", "pos_y_t", "vel_x_t", "vel_y_t", "acc_x_t", "acc_y_t");
	osDelay(1);
	sdlog->Logger_Write("%8s %8s %8s %8s %8s ",//LOG_CONTROL
...
void Logger_Data_Callback(void){
	...
	osDelay(1);
	sdlog->Logger_Write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_POS_Z
			get_baroalt_filt(), pos_control->get_pos_target().z, get_pos_z(), get_vel_z(), get_rangefinder_alt(), get_rangefinder_alt_target());
	osDelay(1);
	...
	osDelay(1);
	sdlog->Logger_Write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_TARGET
			pos_control->get_pos_target().x, pos_control->get_pos_target().y, pos_control->get_vel_target().x, pos_control->get_vel_target().y,	pos_control->get_accel_target().x, pos_control->get_accel_target().y);
	osDelay(1);
	sdlog->Logger_Write("%8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_CONTROL
...
//	s2_printf("accelx:%f,accely:%f,accelz:%f\n ",accel_filt.x,accel_filt.y,accel_filt.z);
//	usb_printf("accelx:%f,accely:%f,accelz:%f,yaw:%f\n ",accel_ef.x,accel_ef.y,accel_ef.z,yaw_deg);
